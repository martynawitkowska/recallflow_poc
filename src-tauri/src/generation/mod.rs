...
const QUIZ_INSTRUCTIONS: &str = "Create a precise RecallFlow quiz from the provided source. Return only valid JSON matching the requested schema. Use unique question IDs, at least two unique non-empty answers per question, and copy every correctAnswers value exactly from answers. single_choice and true_false questions must have one correct answer. true_false answers must be ordered as True, then False. Write concise answer choices that directly answer the question in parallel form. Every choice must be self-contained, grammatical, and clear without transcript fragments, missing referents, speaker-centric phrasing, or slang. Never include choices that are paraphrases of each other. When a question asks for a specific number of cases or items, include that many distinct correct answers. Put reasoning only in the explanation field; never make an answer choice an explanatory sentence or repeat the term being asked about. For a question like What does X mean, use choices like Lower bound, not X means lower bound. Make distractors the same semantic category and level of detail as the correct answer, without clues from wording, length, or grammar. Do not add facts absent from the source.";
...
impl CandidatePrompt {
    fn new(chunk: &segmentation::TranscriptChunk, max_candidates: usize) -> Self {
        const INSTRUCTIONS: &str = "Extract as many distinct durable questions as the source supports, up to candidate_limit. Use only the supplied source. Prefer definitions, principles, mechanisms, comparisons, procedures, and explicitly qualified causal relationships. Exclude classroom logistics, jokes, personal anecdotes, slide navigation, lecture-order questions, and unresolved references. Never say according to the lecture, notes, or speaker. Preserve populations, conditions, uncertainty, and other qualifications. Quote exact evidence from the PRIMARY region. Give every question at least two unique, non-empty answers, and copy each correct answer exactly from answers. Write concise answer choices that directly answer the question in parallel form. Every choice must be self-contained, grammatical, and clear without transcript fragments, missing referents, speaker-centric phrasing such as my array, or slang such as guy. Never include choices that are paraphrases of each other. When a question asks for a specific number of cases or items, include that many distinct correct answers. Put reasoning only in the explanation field; never make an answer choice an explanatory sentence or repeat the term being asked about. For a question like What does X mean, use choices like Lower bound, not X means lower bound. Make distractors the same semantic category and level of detail as the correct answer, without clues from wording, length, or grammar. A single_choice or true_false question has exactly one correct answer; true_false answers must be exactly [\"True\", \"False\"]. An empty candidates array is correct when the source has no independently testable knowledge.";
